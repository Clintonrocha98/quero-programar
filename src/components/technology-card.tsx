...
export default function TechnologyCard({ language }: { language: Technology }) {
  return (
    <Card className="h-full hover:border-metal-500 hover:shadow-lg hover:shadow-metal-900/50 transition-all duration-300">
      <Link to={language.url} className="block h-full p-6 hover:no-underline group">
        <div className="flex items-center gap-4 mb-4">
          <div className={`text-white p-3 rounded-lg font-bold text-lg shadow-sm group-hover:scale-105 transition-transform duration-300`}>
            {getTechnologyIcon(language.id || language.name)}
          </div>
